...
function show_search (el_sura, el_aaya) {
  ...
  const find = () => {
    ...
    if (r.length > 50) {
      ...
      //
      let done
      const all_loaded = () => range(r.length).reduce((acc, i) => acc && aa[i].innerHTML !== '', true)
      //
      for (let i = 0; i < r.length; ++i) {
        load(r[i], r[i], () => {
          aa[i].Q('.aya').innerText = ayat[r[i]].replace(/[#A-Z<>]+/g, '')
          if (!done && all_loaded) { done = true; el_sxr.removeChild(spinner) }  // remove the spinner when all aayaat are rendered
        })
      }
    }
  }
  ...
  const find_wrapper = () => imla ? find() : wait()
